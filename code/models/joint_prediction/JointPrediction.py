...
class JointPrediction_StackedHourglass(nn.Module):
    def __init__(self, arch, num_hm):
        """
        Auxiliary network which predicts flattened matrix using intermediate outputs of the Hourglass
        """
        super(JointPrediction_StackedHourglass, self).__init__()
        ...
        for out_feat in self.fc_arch:
            ...
            self.pytorch_layers.append(nn.Sigmoid())
            in_feat = out_feat

        self.pytorch_layers.append(nn.Linear(in_features=in_feat, out_features=num_hm))
        self.pytorch_layers.append(nn.Sigmoid())
...
class JointPrediction_ViTPose(nn.Module):
    def __init__(self, arch, num_hm):
        """
        Auxiliary network which predicts flattened matrix using intermediate outputs of the Hourglass
        """
        super(JointPrediction_ViTPose, self).__init__()
