...
def calculate_tac_per_sample(y_pred: torch.Tensor, covariance_hat: torch.Tensor,
    ...
    var_loss = torch.zeros((1, dim),device=covariance_hat.device)
    for i in range(dim):
...
def get_tic_covariance(pose_net: Union[ViTPose, Hourglass], pose_encodings: dict, matrix: torch.Tensor,
    ...
    with torch.no_grad():
        if model_name == 'Hourglass':
            grads, hessians = _get_derivatives_hg(pose_encodings, pose_net, use_hessian)
        else:
...
calculate_ll = vmap(calculate_ll_per_sample, in_dims=(0, 0, 0))
